...
function checkRequiredSprites(character) {
    const errors = [];
    const resolve = (stage, mood, ghost) => {
        try {
            (0, sprites_1.spriteFor)(stage, character.id, mood, ghost);
        }
        catch (error) {
    ...
    for (const stage of STAGES) {
        for (const mood of MOODS)
            resolve(stage, mood, false);
    }
    // Ghost (neglect) variant uses the character's ghost sprite for non-egg stages.
    resolve("adult", "sick", true);
    return errors;
}
function checkPngAssets(character) {
    const errors = [];
    const dir = (0, characters_1.getCharacterSpriteDir)(character.id);
    const pngs = (0, node_fs_1.readdirSync)(dir).filter((name) => name.endsWith("@3x.png"));
    for (const file of pngs) {
        const buffer = (0, node_fs_1.readFileSync)((0, node_path_1.join)(dir, file));
        ...
        if (info.width > MAX_PNG_DIMENSION ||
            info.height > MAX_PNG_DIMENSION ||
            info.width < MIN_PNG_DIMENSION ||
            info.height < MIN_PNG_DIMENSION) {
            errors.push(`${where}: ${info.width}x${info.height}px is outside the ${MIN_PNG_DIMENSION}–${MAX_PNG_DIMENSION}px range.`);
        }
    }
    return errors;
}
function main() {
    ...
    let characters;
    try {
        characters = (0, characters_1.listCharacters)();
    }
    catch (error) {
